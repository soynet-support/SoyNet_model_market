...
def parse_args(argv=None):
    ...
    parser.add_argument('--image', default='NY_720x1280.jpg', type=str,
                        help='A path to an image to use for display.')
    parser.add_argument('--images', default=None, type=str,
                        help='An input folder of images and output folder to save detected images. Should be in the format input->output.')
    parser.add_argument('--video',  default=None, type=str, # default='0', type=str,
...
def evaluate(net:Yolact, dataset, train_mode=False):
    ...
    if args.image is not None:
        if ':' in args.image:
            ...
            evalimage(net, args.image)  #inference
        return
...
def weight_extractor(output_path, weights) :
    import numpy as np
    ...
    with open(output_path, 'wb') as f:
        ...
        if backbone == 50:
            wegiht_range_lists = [351, 356, 0, 23, 24, 41, 42, 59, 60, 65, 68, 70, 66, 68, 70, 74, 75, 80, 81, 86, 87,
                                  92, 95, 97, 93, 95,
                                  97, 101, 102, 107, 108, 113, 116, 118, 114, 116, 118, 122, 123, 128, 129, 134, 137,
                                  139, 135, 137,
                                  139, 143, 144, 149, 150, 155, 158, 160, 156, 158, 160, 170, 171, 176, 177, 182, 185,
                                  187, 183, 185,
                                  187, 197, 198, 203, 206, 208, 204, 206, 208, 218, 219, 224, 227, 229, 225, 227, 229,
                                  239, 240, 245,
                                  248, 250, 246, 248, 250, 260, 261, 266, 269, 271, 267, 269, 271, 281, 282, 287, 290,
                                  292, 288, 290,
                                  292, 302, 303, 308, 309, 314, 317, 319, 315, 317, 319, 329, 330, 335, 338, 340, 336,
                                  338, 340, 350
                , 379, 385,  # fpn.lat_layers
                                  385, 391,  # fpn.pred_layers
                                  391, 395,  # fpn.downsample_layer
                                  357, 367,  # proto_net
                                  395, 403,  # prediction_layers
                                  395, 403,  # prediction_layers
                                  395, 403,  # prediction_layers
                                  395, 403,  # prediction_layers
                                  395, 403,  # prediction_layers
                                  403, 404
                                  # 367, 379,  # maskiou_net
                                  # 403, 405  # semantic_seg_conv
                                  ]
        elif backbone == 101:
            wegiht_range_lists = [651, 656, 0, 65,  # 0
                                  68, 70, 66, 68, 70, 128, 131, 133, 129, 131, 133, 150,  # 1
                                  152, 154, 150, 152, 154, 213, 215, 217, 213, 215, 217, 270, 272, 274, 270, 272, 274,
                                  327,  # 2
                                  329, 331, 327, 329, 331, 384, 386, 388, 384, 386, 388, 441, 443, 445, 441, 443, 445,
                                  498,  # 2
                                  500, 502, 498, 500, 502, 555, 557, 559, 555, 557, 559, 594,  # 3
                                  596, 598, 594, 596, 598, 651,
                                  679, 695,  # fpn
                                  657, 667,  # proto_net
                                  695, 703,  # prediction_layers
                                  695, 703,  # prediction_layers
                                  695, 703,  # prediction_layers
                                  695, 703,  # prediction_layers
                                  695, 703,  # prediction_layers
                                  703, 704  # semantic_seg_conv
                                  ]
        else:
            print("There isn't {}, backbone must be 50 or 101".format(backbone))
            exit(-1)

        for i_idx in range(int(len(wegiht_range_lists) / 2)):
            for idx in range(wegiht_range_lists[i_idx * 2], wegiht_range_lists[i_idx * 2 + 1]):  #
                key, w = weight_list[idx]
                if "num_batches_tracked" in key:
                    print(idx, "--------------------")
                    continue
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        soynet_weight = 'yolactpp_resnet50_0909.weights'  # 소이넷 웨이트 파일

        if 1:  # weight download
            weights = net.state_dict()
            weight_extractor(soynet_weight, weights)
